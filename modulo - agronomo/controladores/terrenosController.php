...
class TerrenosController{
	...
	static public function ctrEditarTerreno(){
		...
		if(isset($_POST["editarPerimetro"])){
				...
				preg_match('/^[0-9.]+$/', $_POST["editarPorcentaje_minerales"])){
			   	...
			   	$respuesta = Terrenos::mdlEditarTerreno($tabla,$datos);
	...
	static public function ctrBorrarTerreno(){

		if(isset($_GET["idTerreno"])){

			$tabla ="terrenos";
			$datos = $_GET["idTerreno"];

			$respuesta = Terrenos::mdlBorrarTerreno($tabla, $datos);
			...
			if($respuesta == "ok"){
				...
				echo'<script>
					...
					swal({
						  type: "success",
						  title: "el terreno  ha sido borrada correctamente",
						  showConfirmButton: true,
						  confirmButtonText: "Cerrar"
						  }).then(function(result){
									if (result.value) {
